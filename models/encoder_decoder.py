...
from keras.layers import Dense, Dropout, SimpleRNN, RepeatVector, TimeDistributed
...
class EncoderDecoder(Model):
    ...
    def architecture(self):
        ...
        #
        model = Sequential()
        model.add(SimpleRNN(100, return_sequences=False, recurrent_dropout=0.2, input_shape=(n_input, 1),
                            name='recurrent_layer_0'))
        model.add(RepeatVector(n_output))
        model.add(SimpleRNN(100, activation='tanh', return_sequences=True))
        model.add(TimeDistributed(Dense(100, activation='relu')))
        model.add(TimeDistributed(Dense(n_bins, activation='softmax', name='final_layer')))
    ...
    def fit(self, train, config=None):
        ...
        history = self.model.fit(train_x, y_onehot,
                                          epochs=n_epochs, validation_split=0.2, shuffle=False, batch_size=n_batch)
        return history
